...
3. `sklearn.tree.DecisionTreeClassifier` was used to train the model. To combat the imbalance in our target classes, we used the argument `class_weight = 'balanced` within the DecisionTreeClassifier. We used a 3-fold cross-validation on our training set to find the optimal max_depth between a k of 1 and 30. The optimal max_depth was used to build the final model. Finally, the accuracy of the model was tested on the validation set.
