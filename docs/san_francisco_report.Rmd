...
<img src="../results/figures/SF_crime.png" width="75%">
...
4. `sklearn.tree.DecisionTreeClassifier` was used to train the model. To combat the imbalance in our target classes, we used the argument `class_weight = 'balanced` within the DecisionTreeClassifier. We used a 3-fold cross-validation on our training set to find the optimal max_depth between a k of 1 and 30. The optimal max_depth was used to build the final model. `GridSearch` was used to automatically find the optimal max_depth and k, which was used in training our model. Finally, the accuracy of the model was tested on the validation set using sklearn score's method.

5. The Scikit learn `feature_importances_` attribute returned the Gini importance of each feature and was used to determine the strongest predictors for the target classes. The strongest predictors are the predictors with the highest feature_importance.
