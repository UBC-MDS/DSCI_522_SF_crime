...
author: "Authors: Ian Flores Siaca & Betty Zhou"
output: 
  github_document:
    toc: true
    toc_depth: 2
...
What are the strongest predictors for whether a crime instance in San Francisco resulted in an a "processed or non-processed" resolution?
...
sf_data <- read.csv(here::here("data/san_francisco_clean.csv"))

knitr::kable(head(sf_data))
```

__*Table 1. Preview of San Francisco crime dataset.*__
...
__*Figure 1. An Overall check of the number of examples for each target class in resolution.*__ 
...
__*Figure 2. Density distribution of the resolution for crime incidents reported in San Francisco.*__ Lines that are closer together on the density map indicate higher density.
...
__*Figure 3. Bar graph of category distribution for San Francisco crime instances that resulted in a processed and non-processed resolution.*__
...
__*Figure 4. Time distribution for San Francisco crime instances that resulted in a processed and non-processed resolution.*__
...
__*Table 2. Features used to build the decision tree classifier*__
...
```{r}
words <- c('aggravated', 'aided', 'arrest', 'assault', 'auto', 'automobile', 'base', 'battery', 'building', 'burglary', 'case', 'cocaine', 'disturbed', 'drivers', 'enroute', 'entry', 'forcible', 'grand', 'license', 'life', 'locked', 'lost', 'malicious', 'mental', 'mischief', 'missing', 'occurrence', 'outside', 'person', 'petty', 'possession', 'property', 'recovered', 'revoked', 'robbery', 'rock', 'sale', 'stolen', 'street', 'suspended', 'suspicious', 'theft', 'threats', 'traffic', 'unlawful', 'vandalism', 'vehicle', 'vehicles', 'violation', 'warrant')
 
words_matrix <- matrix(words, ncol=10, byrow = TRUE)
words_df <- as.data.frame(words_matrix, stringAsFactors = FALSE)
knitr::kable(words_df, row.names = FALSE, col.names = NULL)
```

__*Table 3. The 50 most common words extracted from the decription feature.*__
...
3. `sklearn.tree.DecisionTreeClassifier` was used to train the model. To combat the imbalance in our target classes, we used the argument `class_weight = 'balanced` within the DecisionTreeClassifier. We used a 3-fold cross-validation on our training set to find the optimal max_depth between a k of 1 and 30. The optimal max_depth was used to build the final model. Finally, the accuracy of the model was tested on the validation set using sklearn score's method.

4. The Scikit learn `feature_importances_` attribute returned the Gini importance of each feature and was used to determine the strongest predictors for the target classes. The strongest predictors are the predictors with the highest feature_importance.
...
```{r}
features <- read.csv(here::here('data/feature_results.csv'))
features$X <- NULL
...
library(ggplot2)
library(forcats)
ggplot(features, aes(x = fct_reorder(features, feature_importance), 
                     y = feature_importance)) + 
    geom_bar(stat = 'identity') +
    coord_flip() + 
    xlab('Feature') + 
    ylab('Feature Importance') +
    labs(caption = 'Note: Words that appear alone as features indicate the presence or absence of it in the description.') +
    theme_bw()
```

__*Table 4. Top features of the decision tree classifier for San Francisco Crime Data. *__
...
__*Table 5. Recoding of Resolution target response variable.*__
